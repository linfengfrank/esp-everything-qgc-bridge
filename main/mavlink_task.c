...
// UART config
// ---------------------------------------------------------------------------
#define UART_TX_BUF     1024    // outbound ring buffer
#define UART_RX_BUF     4096    // inbound ring buffer (headroom for QGC bursts)

// ---------------------------------------------------------------------------
// MAVLink UDP bridge for QGroundControl
// ---------------------------------------------------------------------------
// The ESP32 is the only reader of the PX4 UART.  It therefore also owns the
// MAVLink bridge: raw bytes read from PX4 are forwarded to QGC, and raw UDP
// packets from QGC are forwarded back to PX4.
//
// Default ports come from Kconfig.projbuild:
//   CONFIG_MAVROS_BRIDGE_IN_PORT   = ESP32 UDP listen/source port, default 8888
//   CONFIG_MAVROS_BRIDGE_OUT_PORT  = QGC UDP listen port, default 14550
//   CONFIG_HOST_IPV4_ADDR          = laptop/QGC IPv4 address
#define MAV_BRIDGE_RX_BUF      1024
#define MAV_BRIDGE_MAX_DRAIN   8       // limit UDP packets per 20 Hz loop

static int s_bridge_sock = -1;
static struct sockaddr_in s_qgc_addr;
static uint32_t s_bridge_px4_to_qgc_bytes = 0;
static uint32_t s_bridge_qgc_to_px4_bytes = 0;
static uint32_t s_bridge_last_log_ms      = 0;
...
// ---------------------------------------------------------------------------
// MAVLink UDP bridge helpers
// ---------------------------------------------------------------------------

static void mavlink_bridge_init(void)
{
    s_bridge_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
    if (s_bridge_sock < 0) {
        ESP_LOGE(TAG, "QGC bridge socket create failed: errno=%d", errno);
        return;
    }

    struct sockaddr_in bind_addr = {
        .sin_family      = AF_INET,
        .sin_port        = htons(CONFIG_MAVROS_BRIDGE_IN_PORT),
        .sin_addr.s_addr = htonl(INADDR_ANY),
    };

    if (bind(s_bridge_sock, (struct sockaddr *)&bind_addr, sizeof(bind_addr)) < 0) {
        ESP_LOGE(TAG, "QGC bridge bind failed on UDP %d: errno=%d",
                 CONFIG_MAVROS_BRIDGE_IN_PORT, errno);
        close(s_bridge_sock);
        s_bridge_sock = -1;
        return;
    }

    int flags = fcntl(s_bridge_sock, F_GETFL, 0);
    if (flags >= 0) {
        fcntl(s_bridge_sock, F_SETFL, flags | O_NONBLOCK);
    }

    memset(&s_qgc_addr, 0, sizeof(s_qgc_addr));
    s_qgc_addr.sin_family = AF_INET;
    s_qgc_addr.sin_port   = htons(CONFIG_MAVROS_BRIDGE_OUT_PORT);
    if (inet_aton(CONFIG_HOST_IPV4_ADDR, &s_qgc_addr.sin_addr) == 0) {
        ESP_LOGE(TAG, "Invalid QGC host IPv4 address: %s", CONFIG_HOST_IPV4_ADDR);
        close(s_bridge_sock);
        s_bridge_sock = -1;
        return;
    }

    ESP_LOGI(TAG, "QGC MAVLink bridge enabled: PX4 UART ↔ UDP %s:%d, local UDP %d",
             CONFIG_HOST_IPV4_ADDR, CONFIG_MAVROS_BRIDGE_OUT_PORT,
             CONFIG_MAVROS_BRIDGE_IN_PORT);
}

static inline void bridge_forward_px4_to_qgc(const uint8_t *buf, int len)
{
    if (s_bridge_sock < 0 || len <= 0) return;

    int sent = sendto(s_bridge_sock, buf, len, 0,
                      (const struct sockaddr *)&s_qgc_addr, sizeof(s_qgc_addr));
    if (sent > 0) {
        s_bridge_px4_to_qgc_bytes += (uint32_t)sent;
    }
}

static void bridge_poll_qgc_to_px4(void)
{
    if (s_bridge_sock < 0) return;

    uint8_t buf[MAV_BRIDGE_RX_BUF];

    // Drain only a bounded number of UDP packets per 50 ms cycle so the
    // Offboard setpoint stream never gets starved during parameter downloads.
    for (int i = 0; i < MAV_BRIDGE_MAX_DRAIN; i++) {
        int len = recvfrom(s_bridge_sock, buf, sizeof(buf), 0, NULL, NULL);
        if (len <= 0) break;

        int written = uart_write_bytes(MAV_UART_PORT, (const char *)buf, len);
        if (written > 0) {
            s_bridge_qgc_to_px4_bytes += (uint32_t)written;
        }
    }
}

static void bridge_periodic_log(uint32_t now_ms)
{
    if (s_bridge_sock < 0) return;

    if (now_ms - s_bridge_last_log_ms >= 5000) {
        ESP_LOGI(TAG, "QGC bridge traffic: PX4→QGC=%lu bytes, QGC→PX4=%lu bytes",
                 (unsigned long)s_bridge_px4_to_qgc_bytes,
                 (unsigned long)s_bridge_qgc_to_px4_bytes);
        s_bridge_last_log_ms = now_ms;
    }
}
...
static void parse_incoming(void)
{
    ...
    // Read all available UART bytes in chunks.  The same raw bytes are both
    // forwarded to QGroundControl and parsed for the ESP32 mission logic.
    size_t available = 0;
    uart_get_buffered_data_len(MAV_UART_PORT, &available);

    while (available > 0) {
        size_t chunk = (available > sizeof(buf)) ? sizeof(buf) : available;
        int to_read = (int)chunk;
        int n = uart_read_bytes(MAV_UART_PORT, buf, to_read, 0);
        if (n <= 0) break;

        bridge_forward_px4_to_qgc(buf, n);
        ...
        for (int i = 0; i < n; i++) {
            ...
            // Only process messages from PX4 (sysid=1, compid=1).  The bridge
            // still forwards every byte to QGC; this filter only affects local
            // ESP32 state updates.
...
void mavlink_task(void *arg)
{
    ...
    // WiFi is already connected before this task is spawned, so the UDP socket
    // can be opened here.  Failure only disables QGC bridging; the ESP32↔PX4
    // companion-control path continues to run.
    mavlink_bridge_init();
    ...
    while (1) {
        ...
        // Forward any QGC/MAVLink UDP packets to PX4 before sending our own
        // Offboard keepalive.
        bridge_poll_qgc_to_px4();
        ...
        // Drain RX buffer, update s_state, and forward PX4 MAVLink to QGC
        parse_incoming();

        bridge_periodic_log(now_ms);
